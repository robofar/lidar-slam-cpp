...
void lo::Config::ReadFromYaml(const std::filesystem::path& yaml_file) {
    if (!std::filesystem::exists(yaml_file)) {
        ...
        try {
            ...
            // Helper lambdas
            auto has = [](const YAML::Node& n, const char* k) -> bool { return n[k] && !n[k].IsNull(); };
            auto get_or_bool = [&](const YAML::Node& n, const char* k, bool d) -> bool { return has(n, k) ? n[k].as<bool>() : d; };
            auto get_or_int = [&](const YAML::Node& n, const char* k, int d) -> int { return has(n, k) ? n[k].as<int>() : d; };
            auto get_or_str = [&](const YAML::Node& n, const char* k, const std::string& d) -> std::string {
                return has(n, k) ? n[k].as<std::string>() : d;
            };
            auto get_or_float = [&](const YAML::Node& n, const char* k, float d) -> float { return has(n, k) ? n[k].as<float>() : d; };
            ...
            if (root["setting"]) {
                ...
                this->stop_frame_thre = get_or_int(s, "stop_frame_thre", this->stop_frame_thre);
            ...
            if (root["process"]) {
                ...
                this->adaptive_range_on = get_or_bool(s, "adaptive_range_on", this->adaptive_range_on);
